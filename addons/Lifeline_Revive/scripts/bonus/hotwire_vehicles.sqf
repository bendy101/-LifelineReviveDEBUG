	...
	// Loop through the damage values to find those with damage = 1 
	{
		...
		if (_damage > 0.5) then {
			if (toLower (_hitParts select _index) find "glass" >= 0) then {
				// diag_log format ["Hit part with damage of 1: %1 ", _hitParts select _index];
				_isSmashed = true;
			};
		};
	} forEach _hitDamages;
...

	{
		...
		if (toLower _hitPart find "glass" >= 0) then {
			...
			if (_damage >= _breakThreshold) then {
				_glassBroken = true;
				diag_log format ["Broken glass detected: %1 with damage %2 (Threshold: %3)", _hitPart, _damage, _breakThreshold];
			};
		};
	} forEach _hitParts;
