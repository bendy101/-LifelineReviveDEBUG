...
LifelinePVEOPFORText2 = "OPFOR in PVE (PVP games automatic)";
...
["Lifeline_Scope", "LIST",     ["Scope", "Scope of mod. Which units will Lifeline Revive affect?\n\n\n"], ["Lifeline Revive","_MAIN"], [[1, 2, 3, 4], ["Group","Playable Slots", "Side", "Choose in Mission"], 0],true] call CBA_fnc_addSetting;
...
// e.g. how easy are they to kill with instant death setting.\n\n"], ["Lifeline Revive",LifelinePVEOPFORText2], true,true] call CBA_fnc_addSetting;
...
'Choose in Mission'.
\n\n"], ["Lifeline Revive",LifelinePVEOPFORText2], false,true] call CBA_fnc_addSetting;
...
if (Lifeline_ACEcheck_ == false) then {
	...
	*Set this to 1 if you want fastest revive: only 1 action needed for any revive, 
	...
	\n\n"], ["Lifeline Revive","_MAIN"], [1, 10, 8, 0],true,{Lifeline_BandageLimit = round Lifeline_BandageLimit}] call CBA_fnc_addSetting;
...
2 = Realism. Instant death on realistic level."], ["Lifeline Revive","_MAIN"], [[0,1,2], ["Off","Moderate", "Realism"], 1],true] call CBA_fnc_addSetting;
	...
	(in Arma, damage is from 0-1, with 1 being lethal)		\n\n"], ["Lifeline Revive","_MAIN"], [0.5, 0.8, 0.7, 1],true,{Lifeline_IncapThres = (round(Lifeline_IncapThres * 10)/10)}] call CBA_fnc_addSetting;
	//OPFOR
...
2 = Realism. Instant death on realistic level."], ["Lifeline Revive OPFOR","_MAIN"], [[0,1,2], ["Off","Moderate", "Realism"], 2],true] call CBA_fnc_addSetting; */
	...
	["Lifeline_InstantDeathOPFOR", "LIST",     ["Instant Death",  "0 = Off. Always go into incapacited state.
1 = Moderate. A bit more casual, instant death still happens with headshots etc.
2 = Realism. Instant death on realistic level."], ["Lifeline Revive",LifelinePVEOPFORText2], [[0,1,2], ["Off","Moderate", "Realism"], 2],true] call CBA_fnc_addSetting;
...
if (Lifeline_ACEcheck_ == false) then {
	...
	*note there is an extra option available in debugging, 
	the revive pair timer called 
	'pair timer for HUD list of units'.
	This is the timeout left before resetting the medic.  \n\n"], ["Lifeline Revive","HUD & MAP"], [[0, 1, 3], ["Off","Names", "Names & distance"], 0],true] call CBA_fnc_addSetting;
};
...
\n\n"], ["Lifeline Revive","HUD & MAP"], [[1, 2, 3, 4, 5, 6, 7], ["Full Name","Last Name", "Unit No, Last Name", "Last Name, Group", "Unit No, Group", "Unit No, Last Name, Group", "Unit No, Full Name, Group"], 5],true] call CBA_fnc_addSetting; */
...
\n\n"], ["Lifeline Revive","HUD & MAP"], [[1, 2, 3, 4, 5, 6, 7], ["Fullname","Surname", "Unit No. / Surname", "Surname / Group", "Unit No. / Group", "Unit No. / Surname / Group", "Unit No. / Fullname / Group"], 5],true] call CBA_fnc_addSetting;

["Lifeline_HUD_namesize", "LIST",     ["Text Size for List", 
"1. Normal
2. Small
3. Smaller
\n\n"], ["Lifeline Revive","HUD & MAP"], [[1, 2, 3], ["Normal","Small", "Smaller"], 0],true] call CBA_fnc_addSetting;
...
New method: no weapon pulled out between bandages - but due to arma bugs - there is a small animation glitch in the loop (frame jump)\n\n"], ["Lifeline Revive","~MISC"], [[0, 1], ["Old Method","New Method"], 1],true] call CBA_fnc_addSetting;

   //MEDIC SELECTION 
// ["Lifeline_Blacklist_MedicOnly", "CHECKBOX", ["Arma Medics Only", "Only designated medics can revive\n\n"], ["Lifeline Revive","MEDIC SELECTION"], false,true] call CBA_fnc_addSetting;
// ["Lifeline_Blacklist_Max", "LIST",     ["Limits.",  "Number of Medics per Group"], ["Lifeline Revive","MEDIC SELECTION"], [[1,2,3,4,5,6,7,8,9], ["No limits","1 unit in Group", "2 units in Group", "3 units in Group", "4 units in Group", "5 units in Group", "20% of Group", "50% of Group", "80% of Group"], 0],true] call CBA_fnc_addSetting;
["Lifeline_LimitDist", "SLIDER",   ["Distance Limit", "Only units within this distance will be become a medic.\nThis means distance from the incap.\nIn metres.\n\n"], ["Lifeline Revive","MEDIC SELECTION"], [200, 4000, 1000, 0],true,{Lifeline_LimitDist = round(_this)}] call CBA_fnc_addSetting;
["Lifeline_Blacklist_Mounted_Weapons", "CHECKBOX", ["Mounted Weapons - Blacklist", "Units with mounted weapons cannot become medics.\nThis includes gunners of vehicles.\n\n"], ["Lifeline Revive","MEDIC SELECTION"], true,true] call CBA_fnc_addSetting;
...
by closing minimap which will also disable Blufor tracking.\n\n"], ["Lifeline Revive","~BONUS. Unrelated to revive but useful"], [[0, 1, 2], ["ACE default (show regardless of GPS)","Only w GPS unit","Only w GPS minimap on"], 1]] call CBA_fnc_addSetting};
    ...
    //~DEBUG
["Lifeline_Revive_debug", "CHECKBOX", ["Debug On", "Debug On. Settings below only work if this is on"], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;
["Lifeline_HUD_dist_font", "LIST",     ["Font for distance hint",  "Font for distance hint"], ["Lifeline Revive","~~DEBUG"], [["EtelkaMonospacePro","PuristaBold","PuristaLight","PuristaMedium","PuristaSemibold","RobotoCondensed","RobotoCondensedBold","RobotoCondensedLight"], ["EtelkaMonospacePro","PuristaBold","PuristaLight","PuristaMedium","PuristaSemibold","RobotoCondensed","RobotoCondensedBold","RobotoCondensedLight"], 0],true] call CBA_fnc_addSetting;

["Lifeline_yellowmarker", "CHECKBOX", ["3D Arrow Markers", "in debug mode, show 3D markers when medic 20 metres away from incap."], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;

["Lifeline_remove_3rd_pty_revive", "CHECKBOX", ["Remove Other Revive Systems Before Mission", "Uncheck this if you want the choice of cancelling Lifeline Revive in the mission.\nNot the best method however, its better to disable mod and restart mission (not restart Arma 3).\nDo this by unchecking 'ENABLE Lifeline Revive' and restarting mission.\n\n"], ["Lifeline Revive","~MISC"], true,true] call CBA_fnc_addSetting;
["Lifeline_hintsilent", "CHECKBOX", ["Debug Hints", "Debug Hints. Using BI 'hinstsilent'"], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;
["Lifeline_debug_soundalert", "CHECKBOX", ["Error Sound Alerts", "Sound Alerts when there is a bug."], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;
["Lifeline_HUD_names_pairtime", "CHECKBOX", ["pair timer for HUD list of units", "incl time for pairs in HUD list of incapped units and medics"], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;
["Lifeline_StartReviveBETA", "CHECKBOX", ["BETA: test version of medic journey to incap", "Still not 100% convinced with my new code for the final 20 metres of medic getting to incap. \nStill testing.  \n\n"], ["Lifeline Revive","~~DEBUG"], false,true] call CBA_fnc_addSetting;
